...
MatrixInfo *read_file(const char *path)
{
    MM_typecode matcode;
    FILE *f;
    ...
    /*  M N NZ represent rowid, columnid and number of non-zero value*/
    int ret_code;
    if ((ret_code = mm_read_mtx_crd_size(f, &M, &N, &nz)) != 0)
        return NULL;
    ...
    /*   specifier as in "%lg", "%lf", "%le", otherwise errors will occur */
    /*  (ANSI C X3.159-1989, Sec. 4.9.6.2, p. 136 lines 13-15)            */
    /*  scan every entry and put the every non-zero value into the array  */
    for (i = 0; i < nz; i++)
    {
        double tmp;
    ...
    if (f != stdin)
        fclose(f);
    //create the instance of MatrixInfo and point to the array list for coo format
    ...
    printf("debug , m n nz %d %d %d\n", mat_inf->M, mat_inf->N, mat_inf->nz);
    return mat_inf;
}

MatrixInfo *read_vector_file(const char *path, int N)
{
    ...
    if (vecdim != N)
    {
        printf("unequal to column num N\n");
        return NULL;
    }
    ...
    mat_inf->cIndex = NULL;
    mat_inf->val = vec;
    printf("vector read m n nz %d %d %d\n", mat_inf->M, mat_inf->N, mat_inf->nz);
    return mat_inf;
}

void freeMatrixInfo(MatrixInfo *inf)
{
    free(inf->rIndex);
    free(inf->cIndex);
    free(inf->val);
...
int writeVect(MatrixInfo *inf, const char *path)
{
    FILE *f;
